...
Follow.prototype.validate = async function(action){
  ...
  if(action == "create"){
      if(doesFollowAlreadyExist){this.errors.push("You are following this user")}
  }
  if(action == "delete"){
...
Follow.isVisitorFollowing = async function(followedId, vistorId){
    let followDoc = await followsCollection.findOne({followedId: followedId, authorId: new ObjectID(vistorId)})
    if(followDoc){
        return true
    } else {
        return false
